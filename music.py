...
class Music:
	...
	def action_download(self, from_, chan, parts):
		url = parts[0]
		try:
			subprocess.check_call(["transmission-remote", "-n", "simark:test", "-w", "/home/simark/music/mount/music", "-a", url])
	...
	def action_youtubeplay(self, from_, chan, parts):
		...
		video = subprocess.check_output(["/usr/local/bin/youtube-dl", "-f", "34","-g", url], universal_newlines = True).split()[0]
