...
class Crazy:
	...
	def usage(self, chan, from_):
		self.irc.privmsg(chan, from_[0] + ": the crazyness level must be an int from %d to %d" % (self.min_lvl, self.max_lvl))
	...
	def on_chanmsg(self, from_, chan, msg):
		...
		try:
			...
			self.irc.privmsg(chan, "I require OP, OP, OP -> /mode #dorsal-fun +o DorsalFunBot")
